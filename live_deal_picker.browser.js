...
function state(){return {active:SKY,rows:[SKY,SOLO],selectCalls:[],verifyCalls:0,failNext:false,expireNext:false,listError:false,delaySelect:false,selectRelease:null,deferNextList:false,listRelease:null,listSnapshot:null};}
async function wire(page,s){
  ...
  await page.route("https://property-spine-api.onrender.com/**",async route=>{
    ...
    if(p==="/operator/properties/select"){
      ...
      if(s.delaySelect)await new Promise(r=>{s.selectRelease=r;});
    ...
    if(p==="/operator/session/revoke")return json(route,200,{ok:true});
...
(async()=>{
  ...
  try{
    ...
    ok(deepBoot.errors.length===0,"the deep-link path has no page errors",deepBoot.errors.join(" | "));await deep.close();
