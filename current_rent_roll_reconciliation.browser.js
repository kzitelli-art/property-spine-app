...
(async () => {
  ...
  try {
    ...
    ok("B4 an already-inventoried property does not offer bulk creation of duplicate new homes, while individual reviewed choices remain", (await page.getByRole("button", { name: "Approve all unresolved as new homes" }).count()) === 0 && /Current inventory is already on record/.test(body) && options.some((o) => /Create new as source names it/.test(o)), body.slice(-900));
    const v = await visible('select[aria-label^="Home for"]');
    ok("B5 the picker is visible to the document (not covered)", v.found && !v.covered);
    ...
    ok("B6 the reviewed mapping shows 4 of 4 resolved before Apply", /4 of 4 current source identities/.test(await page.locator("body").innerText()));
    ...
    ok("B7 rows read: three signed rows are Ready with a blank term; the blank row Needs a look and is not vacancy", (table.match(/\bREADY\b/gi) || []).length === 3 && /needs a look/i.test(table) && /blank name cannot establish vacancy/i.test(table) && /—\s*→\s*—/.test(table), table.slice(0, 600));
    const understood = await page.locator("body").innerText();
    ok("B8 the mapping line names Monthly Rent as rent and the tracker columns as read", /Monthly Rent → actual_rent/i.test(understood) || /Monthly Rent/.test(understood), understood.match(/Columns understood:[^\n]*/) && understood.match(/Columns understood:[^\n]*/)[0]);
    ...
    ok("B10 the bed's current resident is offered as the identity candidate (recognition over re-entry)", (await useBtn.count()) === 1 && /already has a resident on record/i.test(rowText), rowText);
    ...
    rowText = await page.locator('tr:has-text("101A|A")').innerText();
    ok("B11 Add recognises the existing tenancy: Already represented, tied to the lease in force, no second lease", /\bADDED\b/i.test(rowText) && /Already represented/.test(rowText) && (await pool.query("select count(*)::int n from leases where space_id=$1", [bed101_1])).rows[0].n === 1, rowText);
    await shot("06-already-represented.png");
    const blank = await page.locator('tr:has-text("102B|B")').innerText();
    ok("B12 the blank row offers only Leave out — never Add, never vacancy", /leave out/i.test(blank) && !/\bADD\b/i.test(blank.replace(/ADDED/gi, "")), blank);
    ...
    ok("B13 establishing records the position with the unresolved blank row still needing attention", /established as of 2026-09-13/i.test(after) || /Lease and occupancy established/i.test(after) || /needing attention/i.test(after), after.slice(0, 400));
    await shot("07-established.png");
    ok("B14 browser emitted no page errors", errors.length === 0, errors.join("\n"));
  } finally {
