...
ok(/createTeamInvite/.test(code), "comment-stripping left the invite call intact",
...
ok(/if\s*\(\s*!hasStaffSession\(\)\s*\)/.test(code),
   "the action refuses before the sealed staff session exists");
ok(/__psLive\.createTeamInvite/.test(code),
   "the invite delegates to the sealed live-session client");
...
ok(/propertyId\s*:\s*_invitePropertyId/.test(code),
   "the sealed invite action receives the session-derived property");
...
console.log("\n── 3 · honest failure ──");
ok(/if\s*\(\s*!_invitePropertyId\s*\)/.test(code),
...
ok(/Sign in before inviting a teammate/.test(fn),
   "the signed-out path says sign in instead of staging a write");
...
ok(!/Demo invite staged locally/.test(fn),
   "the governed invite has no local success-like staging path");
